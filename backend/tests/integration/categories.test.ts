...
describe('GET /api/categories/:productType', () => {
  it('should respond with status 401 when headers isnt given', async () => {
    const response = await server.get('/api/categories/1');
  ...
  it('should respond with status 401, if token isnt given', async () => {
    ...
    const response = await server.get('/api/categories/1').set('Authorization', '');
  ...
  describe('when token is valid', () => {
    it('should respond with status 400, if params doesnt match with product type name', async () => {
      ...
      const response = await server.get('/api/categories/unknown').set('Authorization', `Bearer ${data.token}`);
    ...
    it('should respond with status 404, if there is no category registered', async () => {
      ...
      const response = await server.get('/api/categories/foods').set('Authorization', `Bearer ${data.token}`);
